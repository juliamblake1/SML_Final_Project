...
```{r}
#setwd("C:/Users/julia/OneDrive/Documents/Columbia/Spring 2024/SML (STAT5241)/Final Proj")

renters_2022 <- read.csv("Data/renters_2022_transformed.csv")
train_data <- read.csv("Data/train_transformed.csv")
test_data <- read.csv("Data/test_transformed.csv")
validation_data <- read.csv("Data/validation_transformed.csv")

predictors <- c("STATEFIP", "SEX", "AGE", "MARST", "BPL", "LANGUAGE", "DENSITY",
                "INCTOT", "RACE", "CITIZEN", "EDUC", "EMPSTAT", "REGION")
response <- "RENTGRS"
...
### Checking normality
Before doing any analysis , let's check if the data is normally distributed to then apply a Gaussian model on it. Otherwise, we may decide to do a transformation , or a different distribution.
...
We can try to use a non -parametric then
```{r}
...
We can see that REGION gave us errors for singularities, so let's remove them for now. 
```{r}
predictors=c("STATEFIP", "SEX" , "AGE" , "MARST" , "BPL", "LANGUAGE","DENSITY", "INCTOT", "RACE", "CITIZEN", "EDUC", "EMPSTAT")
```
...
```{r}
model_gaussian <- glm(log_RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT , 
...
```{r}
model_gaussian_no_link <- glm(log_RENTGRS ~STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT, 
...
Since we had transformed the variable. We assume normality, and can do just gaussian family, it lowered the mse only XX points. 

```{r}
model_gaussian_no_log <- glm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT, 
                   family = gaussian(), data = train)
...
```{r}
model_gaussian_log_link <- glm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT, 
                   family = gaussian(link = "log"), data = train)
...
W_model_gaussian_no_log <- glm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT, 
...
```{r}
model_gamma_link <- glm(RENTGRS ~STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT , 
                   family = Gamma(link = log), data = train)
...
W_model_gamma_link <- glm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT , 
...
```{r}
model_gamma_link <- glm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT +PERNUM +HHMEMBS, 
                   family = Gamma(link = log), data = train)
...
model_gamma_link <- glm(log_RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT + PERNUM + HHMEMBS, 
                   family = Gamma(link = log), data = train)
...
```{r}
linearModel <- lm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT + PERNUM + HHMEMBS, data = train)
summary(linearModel)
...
x <- model.matrix(log(RENTGRS) ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT + PERNUM + HHMEMBS, data = train)
y <- log(train$RENTGRS)
x.test <- model.matrix(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT + PERNUM + HHMEMBS, data = test)

# Fit Ridge Regression
cv_fit <- cv.glmnet(x, y, alpha = 0)  # alpha = 0 for ridge regression
best_lambda <- cv_fit$lambda.min # The best lambda according to cross-validation
ridge_model <- glmnet(x, y, alpha = 0, lambda = best_lambda)

summary(ridge_model)
coef(ridge_model)
cat("Ridge Lambda", ridge_model$lambda, "\n")
cat("Ridge Dev Ratio", ridge_model$dev.ratio, "\n")
...
# Residuals Analysis
residuals_ridge <- test[, response] - ridge_pred
...
plot(sqrt(abs(residuals_ridge)) ~ ridge_pred,
     xlab = "Fitted values",
     ylab = "Square root of absolute residuals",
     main = "Scale-Location Plot")
smoothScatter(ridge_pred, sqrt(abs(residuals_ridge)), colramp = colorRampPalette(c("blue", "red")))
...
summary(lasso_model)
coef(lasso_model)
cat("Lasso Lambda", lasso_model$lambda, "\n")
cat("Lasso Dev Ratio", lasso_model$dev.ratio, "\n")

# Create a scatter plot of actual vs. predicted values
plot(test[, response], lasso_pred, 
     xlab = "Actual Values", ylab = "Predicted Values", 
...
# Residuals Analysis
residuals_lasso <- test[, response] - lasso_pred
...
# Check for homoscedasticity (constant variance)
plot(sqrt(abs(residuals_lasso)) ~ lasso_pred,
     xlab = "Fitted values",
     ylab = "Square root of absolute residuals",
     main = "Scale-Location Plot (Lasso)")
smoothScatter(lasso_pred, sqrt(abs(residuals_lasso)), colramp = colorRampPalette(c("blue", "red")))

# Evaluate Cook's distance for identifying influential points
cooksd_lasso <- cooks.distance(final_lasso_model)
influential_points_lasso <- which(cooksd_lasso > 4 / length(cooksd_lasso))
points(lasso_pred[influential_points_lasso], residuals_lasso[influential_points_lasso], col = "red", pch = 16) # Highlight influential points on the scatter plot
legend("topright", legend = "Influential Points", col = "red", pch = 16)
...
###############################################################################
Now we will run a Bayesian Linear Regression
```{r}
# Bayesian estimation
library(brms)
...
brm_model <- brm(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT,
                 data = train) # can add MCMC settings
...
tree_model <- rpart(RENTGRS ~ STATEFIP + SEX + AGE + MARST + BPL + LANGUAGE + DENSITY + 
                   INCTOT + RACE + CITIZEN + EDUC + EMPSTAT +PERNUM + HHMEMBS, data = train)
